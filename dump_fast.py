...
for match in re.finditer(r'--- !u!224 &(\d+)\nRectTransform:(.*?)(?=\n---|\Z)', text, re.DOTALL):
    ...
    children_matches = re.findall(r'- fileID: (\d+)', content[content.find('m_Children:'):content.find('m_Father:')])
    if not children_matches:
        children_block = re.search(r'm_Children:\s*((?:- \{fileID: \d+\}\s*)*)', content)
        if children_block:
            children_matches = re.findall(r'- \{fileID: (\d+)\}', children_block.group(1))
...
def print_tree(rt_id, depth=0):
    ...
    print("  " * depth + f"- {name} [pos={rt['pos']}, size={rt['size']}, aMin={rt['amin']}, aMax={rt['amax']}, pivot={rt['pivot']}]")
    for child_id in rt['children']:
        print_tree(child_id, depth+1)

for rt_id, rt in rects.items():
    go = objects.get(rt['go_id'])
    if go and go['name'] in ('LeftPart', 'SPECIAL'):
        print_tree(rt_id)
