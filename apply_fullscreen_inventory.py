...
namespace Brotherhood
{
    public sealed class BlasInventoryUI : MonoBehaviour
    {
        ...
        void RefreshTab()
        {
            ...
            string bgPath = "";
            int cols = 5;
            float startX = 1000f; // Right-aligned grid when illustration is on left
            Vector2 graphicPos = new Vector2(400f, -440f);
            Vector2 graphicSize = new Vector2(500f, 440f);

            if (currentTab == 1) // Prayers (Sacred prayer book)
            {
                bgPath = "UI/Sprites/inventory-bg-prayers";
                cols = 5;
                startX = 1000f;
                graphicPos = new Vector2(400f, -440f);
            ...
            else if (currentTab == 3) // Relics
            {
                bgPath = "UI/Sprites/inventory-bg-relicary";
                cols = 5;
                startX = 1000f;
                graphicPos = new Vector2(420f, -440f);
                graphicSize = new Vector2(460f, 460f);
            }
            else if (currentTab == 5) // Collectibles (Bone ossuary chest)
            {
                bgPath = "UI/Sprites/inventory-bg-osary";
                cols = 5;
                startX = 1000f;
                graphicPos = new Vector2(380f, -440f);
                graphicSize = new Vector2(540f, 440f);
            }
            else if (currentTab == 0) // Rosary beads
            {
                bgPath = "UI/Sprites/inventory-bg-rosary";
                cols = 5;
                startX = 1000f;
                graphicPos = new Vector2(420f, -440f);
                graphicSize = new Vector2(460f, 460f);
            ...
            // Reposition slots dynamically in 1920x1080 canvas
            float slotGap = 136f;
            float row0Y = -280f, row1Y = -430f;
            for (int i = 0; i < 14; i++)
            {
                int row = i / cols;
                int col = i % cols;
        ...
        void UpdateDetails()
        {
            ...
            if (itemDescText != null)
            {
                ...
                if (item != null)
                {
                    ...
                    else if (!string.IsNullOrEmpty(item.lore)) desc = item.lore.Length > 250 ? item.lore.Substring(0, 250) + "..." : item.lore;
                }
        ...
        void BuildUI()
        {
            ...
            try
            {
                ...
                rootImg.color = new Color(0.045f, 0.025f, 0.035f, 1f);
                ...
                var catTitle = MakeLabel(win.transform, CategoryTitles[currentTab], 36, titleFont, new Color(1f, 0.88f, 0.45f));
                ...
                lrt.anchoredPosition = new Vector2(400f, -440f);
                lrt.sizeDelta = new Vector2(500f, 440f);
                leftGraphicImg = lgo.AddComponent<Image>();
                leftGraphicImg.preserveAspect = true;
                leftGraphicImg.raycastTarget = false;
                leftGraphicImg.gameObject.SetActive(false);

                // Slot Grid (14 slots instantiated, authentic 112x112 size)
                float slotSize = 112f, slotGap = 136f, gridStartX = 360f, row0Y = -280f, row1Y = -430f;
                ...
                dmRT.anchoredPosition = new Vector2(0, -660f);
                ...
                var descGO = MakeLabel(win.transform, "", 24, bodyFont, new Color(0.88f, 0.82f, 0.65f));
                ...
                dRT.anchoredPosition = new Vector2(120f, -680f);
                dRT.sizeDelta = new Vector2(-400f, 320f);
                ...
                // Lore button (Authentic Boton_Lore.png, 210x66)
                ...
                lbRT.anchoredPosition = new Vector2(-120f, 70f);
                lbRT.sizeDelta = new Vector2(210f, 66f);
                ...
                ebRT.anchoredPosition = new Vector2(-360f, 70f);
                ebRT.sizeDelta = new Vector2(210f, 66f);
...
with open(r'D:\game\Android2D_Blas\Assets\Brotherhood\Runtime\BlasInventoryUI.cs', 'w', encoding='utf-8') as f:
    f.write(code)
print("Updated BlasInventoryUI.cs to authentic FULLSCREEN layout!")
