...
# 1. Update TouchControls.cs BuildHud
...
new_build_hud = ''' void BuildHud()
 {
  ...
  hp=OriginalGauge(hudSafe,"Health",new Vector2(190,-86),200,12,"HUD/HealthFill",false,out hpLoss);
  Image unused;
  fervour=OriginalGauge(hudSafe,"Fervour",new Vector2(190,-102),140,8,"HUD/FervourFill",true,out unused);
  ...
  for(int i=0;i<flaskIcons.Length;i++){
   flaskIcons[i]=AddArt(hudSafe,"Bile flask "+(i+1),"HUD/FlaskFull",new Vector2(0,1),new Vector2(184+i*34,-140),new Vector2(28,44));
...
with open(r'D:\game\Android2D_Blas\Assets\Brotherhood\Runtime\TouchControls.cs', 'w', encoding='utf-8') as f:
    f.write(tc_code)
print("Updated TouchControls.cs BuildHud successfully")
